# Distancia absoluta y relativa entre observables del Universo

**Autor:** [Juan Antonio Lloret Egea](https://es.linkedin.com/in/juanantoniolloretegea/) . **ORCID:** [0000-0002-6634-3351](https://orcid.org/0000-0002-6634-3351) . **Sello editorial:** [ITVIA](https://itvia.online) . **Serie editorial:** IA eñ™ — La Biblia de la IA™ . **ISSN:** [2695-6411](https://portal.issn.org/resource/ISSN/2695-6411) . **Licencia:** [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/) . **Lugar y fecha:** Madrid, 19/05/2026


![Portada](https://raw.githubusercontent.com/juantoniolloretegea/SV-matematica-semantica/main/documentos/adendas/matematica-fisica-factual-contemporanea-sv/interaccion-intercomposicion-transmision-campos/etapa-de-maduracion-matematica-para-version-final/distancia-absoluta-relativa-observables-universo/imagenes/portada.png)

Este directorio contiene el desarrollo completo en formato Markdown para GitHub, con portada, índice general, cuerpo principal, bibliografía y laboratorios.

## Lectura principal

- Documento completo: [`distancia-absoluta-relativa-observables-universo.md`](https://github.com/juantoniolloretegea/SV-matematica-semantica/blob/main/documentos/adendas/matematica-fisica-factual-contemporanea-sv/interaccion-intercomposicion-transmision-campos/etapa-de-maduracion-matematica-para-version-final/distancia-absoluta-relativa-observables-universo/distancia-absoluta-relativa-observables-universo.md)
- Portada: [`imagenes/portada.png`](https://raw.githubusercontent.com/juantoniolloretegea/SV-matematica-semantica/main/documentos/adendas/matematica-fisica-factual-contemporanea-sv/interaccion-intercomposicion-transmision-campos/etapa-de-maduracion-matematica-para-version-final/distancia-absoluta-relativa-observables-universo/imagenes/portada.png)
- Laboratorios: [`laboratorios/`](https://github.com/juantoniolloretegea/SV-matematica-semantica/tree/main/documentos/adendas/matematica-fisica-factual-contemporanea-sv/interaccion-intercomposicion-transmision-campos/etapa-de-maduracion-matematica-para-version-final/distancia-absoluta-relativa-observables-universo/laboratorios)

## Contenido

- `distancia-absoluta-relativa-observables-universo.md` — Desarrollo unificado con portada, resumen, abstract, índice general, seis partes, bibliografía, advertencia y palabras clave.
- `imagenes/portada.png` — Imagen de portada.
- `laboratorios/lab_bancos_negativos.py` — Programa determinista de bancos positivos y negativos.
- `laboratorios/salida_esperada.txt` — Salida esperada resumida.
- `laboratorios/salida_obtenida.txt` — Salida obtenida al ejecutar el laboratorio.
- `registros/` — Reservado para registros de firma y estampillado temporal.
- `pdf/` — Reservado para la versión PDF de la publicación.

## Ejecución de laboratorios

Desde la carpeta `laboratorios/`:

```bash
python lab_bancos_negativos.py
```

El laboratorio debe aceptar 6/6 positivos, rechazar 15/15 negativos por la compuerta correspondiente y emitir el dictamen `BATERÍA SUPERADA`.
