...
El clasificador celular empleado a lo largo de esta publicación es la aplicación determinista:
...
| Registro ℓ | Resultado de Act_D(K,ℓ) |
|---|---|
| Seed_τ(z) o Add_τ(z) | K con z activado en K[τ] |
| Withdraw_τ(z) | K con z desactivado en K[τ] |
| ExecReason(e), ExecComp(e) o Prov(p) | K sin cambios en su proyección activa |
...
Inc_D(E) es el conjunto de los triples (q, τ, [z]rep[D,τ]) que satisfacen simultáneamente las cuatro condiciones siguientes:

1. ℓ_q = Add_τ(z) pertenece a Suf_D(Lᵢ,Lⱼ).
2. (τ, [z]rep[D,τ]) pertenece a Δ⁺_D(ℓ_q).
3. (τ, [z]rep[D,τ]) no pertenece a Hist_D(L<q).
4. Supp_D(ℓ_q,E) es verdadero.
...
| Condición tras completar el procedimiento exhaustivo declarado | DecLearn_D(E*) |
|---|---|
| Un registro retenido y completo respecto de ρ_E establece Learn_D(E) | LEARN |
| Un registro retenido y completo respecto de ρ_E establece ¬Learn_D(E) | NO_LEARN |
| La consulta es admisible, se ha agotado y ninguna base retenida permite clausurar ninguno de los dos veredictos anteriores | U |
...
| Episodio | Razonamiento interno | Aprendizaje trazable |
|---|---|---|
| E_A | verdadero | verdadero |
| E_B | verdadero | falso |
| E_C | falso | verdadero |
| E_D | falso | falso |
...
para el cual N₀ = 8 y N₁ = 1. Puesto que N₀ ≥ T(9), la célula fuente se clasifica como APTO. Sea el conector:

| Resultado de la fuente | φ |
|---|---|
| APTO | 0 |
| NO_APTO | 1 |
| INDETERMINADO | U |

Sea ahora v₂ el vector asociado al *frame* inicial considerado de la célula destino C₂:

v₂ = (1,1,1,0,0,0,0,0,0),

para el cual N₀ = 6 y N₁ = 3. Como ninguno de ambos conteos alcanza T(9) = 7, la célula destino se clasifica inicialmente como INDETERMINADO.

La evaluación de C₁ produce APTO y, por tanto, el conector devuelve φ(APTO) = 0. La composición en serie introduce ese valor en la posición de puente k = 1 de la célula destino. El *frame* inicial de C₂ no se modifica retrospectivamente: la operación genera un *frame* sucesor de C₂ cuyo vector asociado es:

ṽ₂ = (0,1,1,0,0,0,0,0,0).

En este vector sucesor, N₀ = 7 y N₁ = 2. Puesto que N₀ ≥ T(9), la célula destino se reevalúa como APTO.

La operación ejecutada se representa por:

C₁ —σ[1,φ]→ C₂.

Sea e_σ esta ocurrencia concreta de ejecución, y no el operador abstracto σ. Sea Exec_σ(E) la proposición que establece que el episodio E registra una ejecución bien formada que comprende, en orden, la evaluación de la célula fuente, la aplicación del conector φ a su resultado, la incorporación de la salida del conector en la posición de puente declarada de la célula destino, la evaluación del *frame* sucesor de dicha célula y la correspondiente transición legítima de la trayectoria.
...
una representación de conocimiento basada en célula, donde Cconᵣ es una célula ternaria bien formada según la sección 3.1, Πᵣ declara el significado de sus posiciones estructurales y Lᵣ es el registro cognitivo vinculado a la trayectoria del sistema.
...
El programa asociado es una realización ejecutable deliberadamente pequeña de las definiciones finitas. Cumple tres funciones: proporcionar ejemplos concretos serializados; facilitar la reproducción de las distinciones utilizadas en los contramodelos; y detectar divergencias accidentales entre versiones sucesivas de la formalización. El programa no interviene en el establecimiento de los resultados matemáticos. La validez matemática de la sección 5 depende exclusivamente de las definiciones, demostraciones y contramodelos formulados en esta publicación.
...
El laboratorio reproducible correspondiente a esta edición española se incluye en la carpeta hija laboratorios/ del repositorio de la publicación. Conserva la implementación determinista de referencia y sus dieciocho casos, con documentación de uso en español. Su función es proporcionar ejemplos ejecutables y pruebas de regresión de la formalización; no constituye una premisa de los resultados matemáticos.
