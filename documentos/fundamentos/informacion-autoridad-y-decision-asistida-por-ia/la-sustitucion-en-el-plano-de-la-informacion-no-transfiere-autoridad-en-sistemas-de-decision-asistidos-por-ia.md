...
| Debilitamiento | Contraejemplo | Propiedad afectada |
| --- | --- | --- |
| Permitir que INFO introduzca una concesión en A | Una transición informativa inserta una concesión para su propio productor. | TA |
| Permitir la emisión de HumanToken a partir únicamente de una concesión | Se emite un token protegido sin un acto humano admitido. | TB |
| Eliminar la condición subject = principal(token) | Un sujeto distinto ejerce un token vigente. | Confinamiento |
| Eliminar la referencia exacta prior_decision_ref | La resolución se aplica a un historial fabricado o incorrecto. | Linaje |
...
| --- | --- |
| La evidencia externa habilita una concesión determinista existente | Cambia IEnabledAuth; AEnv permanece fija; el servicio autorizado consolida la decisión. |
| External y Service producen el mismo candidato, la misma base y la misma firma | Igualdad en el plano de la información; los perfiles de autoridad permanecen diferentes. |
| HumanReviewRequest | Se registra la solicitud; no se emite ningún HumanToken. |
| Ausencia de acto humano admitido | Se rechaza la consolidación o resolución protegida de U. |
| Acto humano nuevo, token coincidente y base de no clausura | Se admite la consolidación protegida de U. |
| Nueva información para una U no soberana | Puede resolverse sin HumanToken cuando lo permite una regla ordinaria. |
| La gobernanza vincula External como servicio determinista | Se admite la delegación; la autoridad reservada al ser humano continúa no disponible. |
